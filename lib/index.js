...
var getYouTubeIdRegex = /^.*(?:(?:youtu\.be\/|v\/|vi\/|u\/\w\/|embed\/)|(?:(?:watch)?\?v(?:i)?=|\&v(?:i)?=))([^#\&\?]*).*/;
var articles;
...
_fs.default.readFile('data/freecodecamp.article.json', 'utf8', function (err, data) {
  ...
  var embedYouTubePlugin = function embedYouTubePlugin(node, builder, _ref) {
    ...
    var youtubeURLMathched = href.match(getYouTubeIdRegex);
    ...
    var prevNodeMarker = builder.createMarker(node.previousSibling.nodeValue);
    var thisNodeMarker = builder.createMarker(node.text, [anchorMarkup]);
    var nextNodeMarker = builder.createMarker(node.nextSibling.nodeValue);
    var markupSection = builder.createMarkupSection('p', [prevNodeMarker, thisNodeMarker, nextNodeMarker]);
    addSection(markupSection);
    var urlShortId = youtubeURLMathched[1];
  ...
  var returnMobiledocForContent = function returnMobiledocForContent(_ref2) {
    var renderableContent = _ref2.renderableContent,
        _id = _ref2._id;
    count++;
